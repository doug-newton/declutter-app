...
export class ClutterApiService {
  ...
  vote(clutterId: string, vote: 'keep' | 'discard'): Observable<{clutter: Clutter}> {
    return this.http.post<{ clutter: Clutter }>(`http://localhost:3000/api/clutter/${clutterId}/vote`, { vote: vote })
  }
  ...
  delete(clutterId: string): Observable<any> {
    return this.http.delete<any>( `http://localhost:3000/api/clutter/${clutterId}`)
  }
